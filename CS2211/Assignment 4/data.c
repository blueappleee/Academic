...
Key *key_construct(char *in_name, int in_id) {
	...
	// allocates memory for the char array for the name string
	char *keyName = (char *) malloc((strlen(in_name) + 1) * sizeof(char)); // this is the problem line

	// copy the string
	strcpy(keyName, in_name);
...
int key_comp(Key *key1, Key *key2) {
	...
	// if value is zero compare the ids
	if (value == 0) {
		if ((*key1).id < (*key2).id) {
			value = -1;
